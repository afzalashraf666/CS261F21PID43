...
driver = webdriver.Chrome(
    executable_path="C:\ProgramData\Anaconda3\chromedriver\chromedriver.exe",
    options=options,
)
...
def get_songs():
    for idx in range(0, len(genres)):
        ...
        for instance in soup.findAll("div", attrs={"class": "sound__body"}):
            ...
            artist_name = instance.find(
                "div",
                class_="sc-type-light sc-text-secondary sc-text-h4 soundTitle__secondary",
            )
            release_date = instance.find(
                "div", class_=["soundTitle__uploadTime sc-mb-0.5x", "sc-visuallyhidden"]
            )
            genre_tag = instance.find("span", class_=["sc-truncate sc-tagContent"])
            like = instance.find(
                "button",
                class_="sc-button-like sc-button-secondary sc-button sc-button-small sc-button-responsive",
            )
            repost = instance.find(
                "button",
                class_="sc-button-repost sc-button-secondary sc-button sc-button-small sc-button-responsive",
            )

            play = instance.find(
                "span",
                class_=[
                    "sc-ministats sc-ministats-small sc-ministats-plays sc-text-secondary"
                ],
            )
            ...
            release_date_int = [
                int(s) for s in str(release_date).split() if s.isdigit()
            ]
